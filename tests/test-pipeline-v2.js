...
const C = {
  xiboBase: (process.env.XIBO_BASE_URL || 'https://cms.signtral.info').replace(/\/$/, ''),
...
function req(url, opts = {}) {
  return new Promise((res, rej) => {
    ...
    const r = lib.request({
      ...
      path: u.pathname + u.search, method: opts.method || 'GET', headers: hdrs, timeout: 12000
    }, r2 => {
...
function summary() {
  ...
  if (SAVE_REPORT) {
    const report = {
      timestamp: new Date().toISOString(), pass, fail, warn,
